...
function getModelToken(target, options) {
    const { collectionName, token } = (0, createSchema_1.getSchemaToken)(target, options);
    core_1.Store.from(target).set(constants_1.MONGOOSE_MODEL_NAME, collectionName);
    MongooseModels_1.MongooseModels.set(collectionName, target);
...
function createModel(target, schema, name = (0, core_1.nameOf)(target), collection, overwriteModels, connection) {
    ...
    if (entity.isDiscriminatorChild) {
        ...
        const ancestorModel = entity.discriminatorAncestor.get(constants_1.MONGOOSE_MODEL);
    ...
    core_1.Store.from(target).set(constants_1.MONGOOSE_MODEL, model);
    return model;
}
